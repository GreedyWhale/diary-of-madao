...
第一次听说 WebSocket 是在面试中被问到 HTML5 的新特性的时候，时至今日我仍然没有在工作中使用过这个特性，每次遇到类似用户支付完成后，查询用户订单是否支付这种需求都采用的是轮询的方案，隔一段时间请求一次查询接口，当我提出可以使用 WebSocket 的时候，后端同事都会说他们那边实现起来很麻烦，正好 Node.js 可以实现 WebSocket 服务端，记录一下实现过程，争取早日用到:-)。
